...
async def host_pipeline(hosts: List[str], names: str, save_path: str, log: bool) -> None:
    await asyncio.gather(*[check_host(host=hosts[idx], name=names[idx], save_path=save_path, log=log) for idx in range(len(hosts))])
