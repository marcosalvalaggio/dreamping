...
class DreamPingApp(QMainWindow):
    ...
    def play(self):
        ...
        if delay == None:
            ...
            print(hosts)
            if save_path:
                def thread_play(delay, save_path, hosts, names):
                    self.running_status = True
                    while self.running_status:
                        asyncio.run(host_pipeline(hosts=hosts, name=names, save_path=save_path))
                        asyncio.sleep(delay)
                play_thread = threading.Thread(target=thread_play)
                play_thread.start()
